...
def get_env_params(max_nb_objects=3,
    ...
    types = tuple(OrderedDict.fromkeys(types)) # filters doubles, when some categories include others.
    nb_types = len(types)
    ...
    def get_obj_type(all_obj_features, i_obj):
        ...
        encoding_list = type_encoding.tolist()
        if 1 in encoding_list:
            ind = encoding_list.index(1)
            obj_type = types[ind]
        else:
            obj_type = None
        return [obj_type]
